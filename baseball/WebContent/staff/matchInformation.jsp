...
<body>
	...
	<c:when test="${fn:length(matchList)> 0}">
		...
		<div id="app">
			...
			<%-- 日付のテーブル --%>
			<table border="1" v-for="(match, index) in filteredMatchList" :key="index">
	    		<tr><th>開催日</th><th>試合順</th><th>高校名</th><th>高校名</th><th>試合状況</th><th>第何回戦</th><th>販売日時</th></tr>
	          	<tr v-for="(duel, j) in duelList[selectIndex]" :key="j">
					<td rowspan="4" v-if="j == 0"><a :href="'MatchUpdateInput?date=' + match.eventDate">{{ match.eventDateStr }}</a></td>
	          		<td>{{j+1}}試合目</td>
		...
		<script>
			new Vue({
				...
				methods:{
				    ...
				    <%-- 現在の日付の試合があればそれを表示・ないなら一番昔の日付を表示 --%>
				    initializeEventDate(){
				    	let found = false;
