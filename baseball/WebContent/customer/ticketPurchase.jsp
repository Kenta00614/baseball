...
<body>
	...
	<%-- 第何回　～大会のヘッダー --%>
	<c:if test="${fn:length(match) > 0}"><h1 class="main_display">第${tour.ordinalNum }回${tour.name }</h1></c:if>

    <%-- 試合日情報がないとき --%>
    <c:if test="${fn:length(match) <= 0}"><h2 class="main_display">販売中のチケットはありません</h2></c:if>
